#
# calculates IVIM parameters by MLE fitting the formula:
...
#    - scipy
#    - nibabel
#    - BayesicFitting 2.3.0 (requires python>=3.5, depends on matplotlib, astropy)
#    - does not work with pyinstaller<=3.5
...
from IVIM_Model import IVIM_Model
...
def FIT (x, y):
    mdl = IVIM_Model()
    #Amoeba is a Maximim Likelihood Estimation (MLE) Fitter
    fitter1 = AmoebaFitter( x, mdl, errdis="gauss") # "gauss" or "cauchy" or "poisson"
    initials = [np.max(y), 0.01, 1e-3, 1./1e-2] 
    mdl.parameters = initials     
    try:   
        param = fitter1.fit( y )
        a=fitter1.parameters[0]
        f=fitter1.parameters[1]
        dslow=fitter1.parameters[2]
        dfast=1./fitter1.parameters[3]      
    except: a=0; f=0; dslow=0; dfast=0 
    if dslow>4e-3 or dslow<0.2e-3: #try again with slightly different initials 
        initials = [np.max(y), 0.1, 1e-3, 1./1e-2] 
        mdl.parameters = initials
        try: 
            param = fitter1.fit( y )
            a=fitter1.parameters[0]
    ...
    if dslow>4e-3: dslow=4e-3 #clip         
    return a, f, dfast, dslow  # fast/slow order inverted   
...
def worker_curvefit(bval, IMGdata, p):
    ...
    for i in range(IMGdata.shape[0]):
       ...
       if nz[0].shape[0]>=4: # need at least 4 valid points 
          B_nz = bval[nz]
...
if __name__ == '__main__':
    ...
    #fit MLE
    print ('\nMLE fitting using', cores, 'cores'); sys.stdout.flush()
