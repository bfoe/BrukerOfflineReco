...
if FIDfileRef == "": print ('ERROR: No FID input file specified'); sys.exit(2)
FIDfileRef = os.path.abspath(FIDfileRef) 
TKwindows.update()
...
#alternative way
#temp = np.sum(np.abs(FIDdataRef[:,:,:,:]),axis=(1))
#ind = np.unravel_index(temp.argmax(), temp.shape)
#max_echo_pos_x = ind[0]; max_echo_pos_y = ind[1]; max_echo_pos_z = ind[2]
...
#zero fill (reference)
SpatResol=METHODdataRef["PVM_SpatResol"]/zero_fill
...
#Phase correction using reference
