# multi core version of "NIFTI_PhaseUnwrap.py"
#
...
def worker (k_start, k_end,IMGdata_decoded_PH_NZ, PhUnwrap_combinations, mask_NZ, all_nonzero, venc_arr, nfiles, dim):
    log_string=""
    dim_reduced = IMGdata_decoded_PH_NZ.shape[0]
    marker_each = int(dim_reduced/70)     
    Img_PH_flow_NZ = np.zeros (shape=(k_end-k_start),dtype=np.float32)
    for k in range (k_start, k_end):
       ...
       #if local_nfiles == 1: Img_PH_flow_NZ [k-k_start]=IMGdata_decoded_PH_NZ[k,nz]
       #else:   
       if local_nfiles >= 3:
          ...
          Img_PH_flow_NZ [k-k_start] = avg[min_chi_index]
          # sucessively add all info to log_string
          ...
          if not np.array_equal(IMGdata_decoded_PH_NZ[k,nz],PhUnwrap_try[min_chi_index,:]):
              ...
              log_string += str(np.unravel_index(all_nonzero[k],dim[0:3]))+'\t'
              log_string += np.array_str(IMGdata_decoded_PH_NZ[k,:])+'\t'+np.array_str(temp)+'\t'
              log_string += str("%7.2f" % Img_PH_flow_NZ[k-k_start])+'\n'              
    return Img_PH_flow_NZ, log_string
...
if __name__ == '__main__':
    ...
    #flatten vectors
    mask = mask.reshape (dim[0]*dim[1]*dim[2],dim[3]); 
    IMGdata_decoded_PH = IMGdata_decoded_PH.reshape (dim[0]*dim[1]*dim[2],dim[3]); 
    ...
    dim_reduced = IMGdata_decoded_PH_NZ.shape[0]
    #do unwrap with multiple cores
    cores=mp.cpu_count()-1; cores = max (1,cores)
    cores=min(cores,dim_reduced) # just in case (don't allocate unnecessary cores)
    print ('Starting  Phase unwrap using %d cores ' % cores)
    p = mp.Pool(cores)
    return_vals=[]
    workpiece=int(math.ceil(float(dim_reduced)/float(cores)))
    for i in range(0,cores):    
        k_start = i*workpiece
        k_end   = k_start+workpiece
        if k_end > dim_reduced: k_end = dim_reduced
        return_vals.append(p.apply_async(worker, args = (k_start,k_end,IMGdata_decoded_PH_NZ, PhUnwrap_combinations, mask_NZ, all_nonzero, venc_arr, nfiles, dim)))
    p.close()
    p.join()
    #get results back from workers
    Img_PH_flow_NZ = np.zeros (shape=(0),dtype=np.float32)
    for i in range(0,cores):
        Img_PH_flow_NZ = np.concatenate ((Img_PH_flow_NZ, return_vals[i].get()[0]), axis=0)
        logfile.write(return_vals[i].get()[1]) # write logstring directly to logfile
    logfile.close()
    print ('')
    #put data structure back to normal
