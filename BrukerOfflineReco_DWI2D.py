...
# this version is for the Standard 2D diffusion method only
#
# output:
...
def FFT2D (array):      
    return np.fft.fft2(array, axes=(0,3)) 

def iFFT2D (array):  
    return np.fft.ifft2(array, axes=(0,3)) 
...
if  METHODdata["Method"] != "DtiStandard"  or METHODdata["PVM_SpatDimEnum"] != "2D":
    print ('ERROR: Recon only implemented for Standard 2D Diffusion method'); 
    sys.exit(1)
...
order1=METHODdata["PVM_EncSteps1"]+dim[3]/2                        
...
FIDdata_ZF = np.zeros(shape=(int(dim[0]*zero_fill),dim[1],dim[2],
                             int(dim[3]*zero_fill)),dtype=np.complex64)
dim0start=int(dim[0]*(zero_fill-1)/2)
dim3start=int(dim[3]*(zero_fill-1)/2)
FIDdata_ZF[dim0start:dim0start+dim[0],:,:,dim3start:dim3start+dim[3]] = \
    FIDdata[0:dim[0],:,:,0:dim[3]]
FIDdata=FIDdata_ZF;
...
#import pylab as pl
#pl.figure()
#pl.imshow(np.log(np.abs(FIDdata[:,dim[1]/2,0,:])))
#pl.show()
#sys.exit(0)
...
IMGdata = np.fft.fftshift(IMGdata, axes=(0,3))
...
dim3start=int((dim[3]-dim[3]/crop[1])/2)
dim0end = int(dim0start+dim[0]/crop[0])
dim3end = int(dim3start+dim[3]/crop[1])
IMGdata = IMGdata[dim0start:dim0end,:,:,dim3start:dim3end]
dim=IMGdata.shape
print('.', end='') #progress indicator

# align first direction (1DOG registration, resolution 1 pixel)
if METHODdata["PVM_DiffPrepMode"]!="DoubleSpinEcho":
    N = 10 #neighbouring pixels  to search
    n_B0 = METHODdata["PVM_DwAoImages"]
    for k in range(n_B0,dim[2]):
        max_cor = 0.0; best = 0    
        for i in range(-N,N):
...
if METHODdata["PVM_SPackArrSliceOrient"] == "axial":
    ...
    IMGdata = np.transpose (IMGdata, axes=(0,3,1,2))
...
#write B0
n_B0 = METHODdata["PVM_DwAoImages"]
NIFTIimg_B0 = nib.Nifti1Image(np.average(IMGdata_ABS[:,:,:,0:n_B0],axis=3).astype(np.int16), aff)
NIFTIimg_B0.header.set_slope_inter(max_ABS/32767.,0)
NIFTIimg_B0.header.set_xyzt_units(3, 8)
NIFTIimg_B0.set_sform(aff, code=0)
NIFTIimg_B0.set_qform(aff, code=1)
nib.save(NIFTIimg_B0, os.path.join(os.path.dirname(FIDfile),OrigFilename+'_B0.nii.gz'))
print('.', end='') #progress indicator

#write B>0
for i in range(1,bvec.shape[0]):
  start = n_B0+i-1
  step=int(METHODdata["PVM_DwNDiffExpEach"])  
  NIFTIimg_B = nib.Nifti1Image(np.average(IMGdata_ABS[:,:,:,start::step],axis=3).astype(np.int16), aff)
  NIFTIimg_B.header.set_slope_inter(max_ABS/32767.,0)
  NIFTIimg_B.header.set_xyzt_units(3, 8)
  NIFTIimg_B.set_sform(aff, code=0)
  NIFTIimg_B.set_qform(aff, code=1)
  nib.save(NIFTIimg_B, os.path.join(os.path.dirname(FIDfile),OrigFilename+'_B'+str(bvec[i])+'iso.nii.gz'))
print('.', end='') #progress indicator
  
#write ADC
tmp1 = np.average(IMGdata_ABS[:,:,:,0:n_B0],axis=3)      # B0 average
nz = np.nonzero (tmp1)
tmp2 = np.average(IMGdata_ABS[:,:,:,start::step],axis=3) # last B average
ADC = np.empty (tmp1.shape, dtype=np.float32)  
ADC [nz] = -1.0*np.log(tmp2[nz]/tmp1[nz])/bvec[-1]
ADC [ADC<=0] = 0 # mask negative values (this happens where tmp2>tmp1, which cannot be true)
ADC *= mask # mask noise
print('.', end='') #progress indicator
#filtering
ADC = median_filter  (ADC, size = (3,3,1))
for i in range(ADC.shape[2]): 
  ADC[:,:,i] = gaussian_filter(ADC[:,:,i], sigma=0.7, truncate=3)
print('.', end='') #progress indicator
#convert unit to 10^-3 mm^2/s
ADC *= 1e3 
#usual in-vivo values:
...
#    CSF: 3.0-3.5
#    restriction (e.g. tumor, stroke) <0.7 
NIFTIimg_ADC = nib.Nifti1Image(ADC, aff)
NIFTIimg_ADC.header.set_slope_inter(1,0)
NIFTIimg_ADC.header.set_xyzt_units(3, 8)
NIFTIimg_ADC.set_sform(aff, code=0)
NIFTIimg_ADC.set_qform(aff, code=1)
nib.save(NIFTIimg_ADC, os.path.join(os.path.dirname(FIDfile),OrigFilename+'_ADC.nii.gz'))
